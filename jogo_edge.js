...
sym.$("part").draggable();sym.$("targ").droppable({drop:function(event,ui){$(ui.draggable).fadeOut();}});sym.$("targ2").droppable({drop:function(event,ui){alert("Bingo");}});sym.$("targ3").droppable({drop:function(event,ui){alert("Bingo");}});var gui=require('nw.gui');$(document).on('click','a',function(event){event.preventDefault();gui.Shell.openExternal($(this).attr('href'))
